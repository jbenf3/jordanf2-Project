...
class Player:
	...
	def __init__(self, name, points = 0, games = 0, sets = 0, set_1 = 0, set_2 = 0, set_3 = 0, df = 0, tpts = np.array([]), tfs = np.array([]), fsi = np.array([]), tss = np.array([]), ssi = np.array([]), errors = 0, winners = 0):
		self.pts = points
        	self.games = games
		self.sets = sets
		self.totalpoints = tpts
		self.tfs = tfs
        	self.fsi = fsi
        	self.tss = tss
        	self.ssi = ssi
